...
def collect(app_dir: Path, repo_root: Path):
    ...
    try:
        ...
        count_asd101 = conn.execute(
            "SELECT COUNT(*) FROM students WHERE subject_code = ?", ("ASD101",)
        ).fetchone()[0]
    finally:
        conn.close()
    ...
    for student in students:
        ok, message = _validate_student(student)
        if not ok:
            return False, message

    return True, (
        "Database evidence: students table has 10 valid rows; "
        f"ASD101 rows count is {count_asd101}; fields are "
